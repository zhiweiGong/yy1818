...
//import com.android.EgLauncher.SoundRecorderActivity;
...
public class AudioPlayWav implements Runnable{
	...
	public AudioPlayWav(Context context, Handler handler, String passwd, File wav){
		...
		mAudioTrack = new AudioTrack(AudioManager.STREAM_MUSIC, sampleRate,
				AudioFormat.CHANNEL_OUT_MONO, audioEncoding,  4096,
		...
		try {
			...
			msg.arg1 = (int)(mRaf.length() - 44+BYTEPERSEC/2) / BYTEPERSEC;

		} catch (IOException e) {
			e.printStackTrace();
		}
		msg.what = MainActivity.STATE_PLAY_STARTED;
	...
	public synchronized boolean seekTo(int sec){
		try {
			if (sec < 0 || sec > ((mRaf.length() - 44+BYTEPERSEC/2) / BYTEPERSEC)){
				return false;
			}
		} catch (IOException e) {
			e.printStackTrace();
...
//			d("this (i + mPos) % mEnCryLen is " + ((i + mPos) % mEnCryLen));
	...
	public long getPlayTimeInSec(){
		try {
			if (mRaf.getFilePointer() >44) {
				long setint = (mRaf.getFilePointer() - 44 ) / BYTEPERSEC;
				float set = ((float)(mRaf.getFilePointer() - 44)) / BYTEPERSEC;
				Log.e(TAG, "second float:" + set + "second int:" + setint);
				return setint;
			} else
				return 0;
		} catch (IOException e) {
	...
	public synchronized void stop(){
		...
		mAudioTrack.release();
		mUiHandler.sendEmptyMessage(MainActivity.STATE_PLAY_END);
	}
