package com.android.EgLauncher.util;
...
public class Fm1388Util {
    ...
    public static final String SYSTEM_LED_PATH = "/sys/class/leds/led-sys/brightness";
	...
	public static void changeModeVr() {
		new Thread(new Runnable() {
			@Override
			public void run() {
				String tCrrentMode = readSysfs(SYSTEM_LED_PATH);
    ...
    public static void SetLed(final boolean status) {
        new Thread(new Runnable() {
            ...
            public void run() {
                if (status)
                    writeSysfs(SYSTEM_LED_PATH, "1");
                else
                    writeSysfs(SYSTEM_LED_PATH, "0");
            }
        }).start();
    }
