...
public class UtilNtpTime {
	private final String TAG = "UtilNtpTime";

	private static UtilNtpTime mUtilNtpTime = null;

	public static synchronized UtilNtpTime getInstance(){
		if(mUtilNtpTime == null){
			mUtilNtpTime = new UtilNtpTime();
		}
		return mUtilNtpTime;
	}
	...
	public void SetNtpTime(Context context){
        SntpClient client = new SntpClient();
        if (client.requestTime("pool.ntp.org", 30000)) {
            long now = client.getNtpTime() + System.nanoTime() / 1000 - client.getNtpTimeReference();
            ((AlarmManager) context.getSystemService(Context.ALARM_SERVICE)).setTime(now);
        }
	}

	public long getNtpTime(Context context){
		long now = 0L;
		SntpClient client = new SntpClient();
		if (client.requestTime("pool.ntp.org", 30000)) {
